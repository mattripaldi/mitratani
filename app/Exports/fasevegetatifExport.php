...
class fasevegetatifExport  implements WithEvents
{
    ...
    public function registerEvents(): array
    {
        return [
            BeforeWriting::class => function(BeforeWriting $event) {
                $templateFile = new LocalTemporaryFile(public_path('excel/fasevegetatif.xlsx'));
                $event->writer->reopen($templateFile, Excel::XLSX);
                $sheet1 = $event->writer->getSheetByIndex(0);

                $this->vegetatif($sheet1);
    ...
    private function vegetatif($sheet){
