...
class faseberbungaExport  implements WithEvents
{
    ...
    public function registerEvents(): array
    {
        return [
            BeforeWriting::class => function(BeforeWriting $event) {
                $templateFile = new LocalTemporaryFile(public_path('excel/faseberbunga.xlsx'));
                $event->writer->reopen($templateFile, Excel::XLSX);
                $sheet1 = $event->writer->getSheetByIndex(0);

                $this->faseberbunga($sheet1);
    ...
    private function faseberbunga($sheet){
